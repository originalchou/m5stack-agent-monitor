...
- `subagents` — named from the main agent's `collaborationspawn_agent` (`/root/<task>`
  in its JSON-string `tool_response`) and status-synced from `collaborationwait_agent`
  (roster), keyed by agent_name. Each spawn's name is bound (FIFO) to the `agent_id` of
  the `SubagentStart` that follows it, so `SubagentStop` flips that subagent to stopped
  mid-turn; `Stop` marks any still-running subagents stopped as a fallback.
